...
class TestInventory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.p3=inventory_informa("flower", 10, 3.1, 10.5, (dt.today()+timedelta(days=3)).strftime("%Y%m%d"))
        self.p4=inventory_informa("snack", 10, 5.5, 13.7, (dt.today()+timedelta(days=20)).strftime("%Y%m%d"))
    def testprofit(self):
        self.assertEqual(self.p1.profit, 3640)
        self.assertEqual(self.p2.profit, 8000)
        self.assertEqual(self.p3.profit, 74)
        self.assertEqual(self.p4.profit, 82)
    def testupdate(self):
